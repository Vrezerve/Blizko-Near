...
- [x] Пункт меню «Уведомления»: цветной индикатор подписки без клика (зелёный = подписан, красный = не подключён, серый = неизвестно/не настроено); статус грузится при открытии меню; сообщение при неподписке — «✗ Вы не подключены к оповещениям» + автоподписка
- [x] Собственные in-app уведомления со звуком (без OneSignal): /app/frontend/src/lib/soundAlert.js (Web Audio API — без файлов). Водитель: новая заявка → тройной восходящий сигнал x2 + вибрация (детект новых id в polling /orders/active, только когда «на линии»). Пассажир: «водитель найден» и «поездка завершена» → двухтональный сигнал + вибрация (переходы статуса через lastOrderStatusRef). Разблокировка аудио по первому клику/тапу (initSoundUnlock). Бонус: системное браузерное уведомление, если вкладка в фоне и разрешение уже выдано (showLocalNotification)

## Большой пакет — July 2026 (iteration 27, все 14 пунктов)
- [x] Редактируемые тексты интерфейса: settings.ui_texts (12 ключей, /app/frontend/src/lib/uiTexts.js UI_TEXT_DEFS + getText), блок «Тексты интерфейса» в админке
- [x] Новые дефолты текстов: «Забыли код? Восстановить», «Найти поездку», «Вход для заказчика», «Адрес и номер дома», «Уточнение», «Разместить заказ», «Ищем исполнителя», «Исполнитель найден», «Выходите через ~N мин», «Позвонить исполнителю»
- [x] SEO: seo_title/seo_description в админке, применяются как document.title и meta description (App.js MapStyleLoader)
- [x] Чекбоксы политик перенесены на ПЕРВЫЙ экран входа водителя (Продолжить заблокировано без галочек)
- [x] Уточнение (бывш. номер дома) — необязательное поле, видно водителю после принятия заказа (без префикса «д.»)
- [x] Подсказки адреса: выпадающий список отключён по умолчанию, тумблер в админке (address_suggestions_enabled)
- [x] БАГ: водители «всегда онлайн» — heartbeat last_seen_at (обновляется при polling /orders/active и toggle-ready), /drivers/stats и online-locations считают только активных за 120 сек
- [x] БАГ: заказ висит бесконечно — expire_stale_pending_orders() (order_auto_cancel_minutes, дефолт 15, настройка в админке), cancelled_by='timeout', заказчику показывается «Исполнитель не найден»
- [x] БАГ (критич.): экран заказчика не обновлялся при принятии/завершении заказа — stale closure orderState в pollOrderStatus, исправлено через orderStateRef; /orders/my-active теперь возвращает завершённый/отменённый заказ 3 минуты (клиент видит финальный статус). Подтверждено E2E-реалтайм тестом
- [x] Лог браузеров: /app/frontend/src/lib/clientLog.js (window.onerror, unhandledrejection, загрузка >5 сек) → POST /api/logs/client → system_logs категория 'browser' (видно в админке в логах)
- [x] Урок: НЕ делать несколько параллельных search_replace на ОДИН файл — часть правок теряется молча (потерялись поля SettingsUpdate и searching_title, найдено testing agent)
- Тесты: /app/test_reports/iteration_27.json — backend 9/10 (критикал с моделью исправлен и перепроверен curl), frontend 8/8 включая реалтайм E2E
- [x] Вкладка «Тексты» в админке: UI_TEXT_GROUPS (~40 ключей в 6 группах: выбор роли, вход/PIN, создание заказа, поездка, экран исполнителя, меню) в /app/frontend/src/lib/uiTexts.js; все компоненты (RoleSelect, CustomerAuth, DriverAuth, PinScreen, CustomerMain, DriverMain, PushCheckMenuItem label prop) используют getText; загрузка settings на вкладке texts (case 'texts' в loadData)
- [x] Toast (sonner) вместо alert в админке: сохранение настроек/текстов, удаление пользователя, загрузка иконки/файлов/модулей, тест письма/push (~26 замен). Диагностические вердикты push оставлены alert (длинные инструкции)
- Проверено: вкладка отображается, сохранение показывает зелёный toast, кастомный текст мгновенно применяется на главной, дефолты вернул; заказчик/водитель/меню рендерятся корректно
...
- [x] Фикс перепутанных согласий (iteration 28, все проверки 8/8): 1) ссылка «условия сервиса» у исполнителя открывает terms_text (была driver_rules_text); 2) «обработку ПЕРСОНАЛЬНЫХ данных» + заголовок окна «Согласие на обработку персональных данных»; 3) заказчик: «правилами для заказчика» / окно «Правила для заказчика»; 4) водитель: «правилами для исполнителей» / окно «Правила для исполнителя»; 5) «Вы не онлайн» + кнопка «Начать принимать заказы». Все строки — редактируемые ключи ui_texts (10 новых полей во вкладке «Тексты»). Примечание: СОДЕРЖИМОЕ окон (terms_text/privacy_text/customer_rules_text/driver_rules_text) редактируется на вкладке «Настройки», заголовки/ссылки — на вкладке «Тексты»
