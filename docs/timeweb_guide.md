...
### Способ 1.5: PHP Веб-Инсталятор (через браузер)
- [Установка через install.php](#способ-15-установка-через-php-веб-инсталятор)
...
# Способ 1.5: Установка через PHP Веб-Инсталятор

> Если вам удобнее работать через браузер, а не терминал — используйте PHP-инсталятор. Он делает всё то же самое что `install.sh`, но с красивым веб-интерфейсом.

---

## Что делает PHP-инсталятор?

| Шаг | Что происходит | Автоматически? |
|-----|---------------|:-:|
| 1 | Проверяет Node.js, Python, MongoDB, Nginx | Да |
| 2 | Устанавливает недостающее ПО | Да (кнопка) |
| 3 | Вы вводите домен, email/пароль админа | Вы вводите |
| 4 | Настраивает бэкенд (venv, pip, .env) | Да |
| 5 | Собирает фронтенд (yarn, build) | Да |
| 6 | Создаёт systemd-сервис + Nginx + SSL | Да |
| 7 | Финальная проверка всех сервисов | Да |

---

## Предварительные требования

- VDS сервер (Ubuntu 22.04) с **root-доступом**
- Установленный **PHP** (минимум 8.0): `apt install php-cli`
...
Загрузите весь проект на сервер (например, в `/opt/taxi-app/`):

```bash
# Через SCP с вашего компьютера
scp -r taxi-app/ root@ВАШ_IP:/opt/taxi-app/
```

Или через файловый менеджер ISPmanager.

Убедитесь что `install.php` лежит в корне проекта:

```
/opt/taxi-app/
├── install.php      <-- инсталятор
├── backend/
│   ├── server.py
│   └── requirements.txt
├── frontend/
...
## Шаг 2: Установите PHP (если ещё нет)
...
## Шаг 3: Запустите инсталятор

```bash
cd /opt/taxi-app
php -S 0.0.0.0:8080 install.php
```

> **Важно:** Команда запускает временный веб-сервер на порте 8080.
...
```
http://ВАШ_IP:8080/install.php
```

Вы увидите пошаговый визуальный инсталятор.

---

## Шаг 5: Следуйте шагам в инсталяторе

1. **Проверка системы** — инсталятор покажет что установлено, а что нет
2. **Установка компонентов** — нажмите «Установить» для недостающих
3. **Настройки** — введите домен/IP, email и пароль администратора
4. **Сборка** — бэкенд и фронтенд настроятся автоматически (3-5 минут)
5. **Сервисы** — systemd, Nginx, SSL настроятся автоматически
6. **Финальная проверка** — убедитесь что всё зелёное

---

## Шаг 6: Удалите install.php!

После успешной установки **обязательно** удалите инсталятор:

- Нажмите красную кнопку «Удалить install.php» в интерфейсе
- Или вручную: `rm /opt/taxi-app/install.php`
- Остановите временный PHP-сервер: `Ctrl+C` в терминале
...
- Сайт: `http://ВАШ_ДОМЕН`
- Админ-панель: `http://ВАШ_ДОМЕН/admin/login`
- Тестовый OTP-код: `1234`
