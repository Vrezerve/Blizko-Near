...
const CustomerAuth = () => {
  ...
  const { sendCode, verifyCode, applyAuthResult, deviceId, user } = useAuth();
  
  const [step, setStep] = useState('phone'); // phone, code, call, blocked
  ...
  // Poll callcheck status while on the 'call' step
  useEffect(() => {
